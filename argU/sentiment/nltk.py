# used datasets: nltk twitter samples,
...
def run(rows, mode):
    ...
    ratio = int(0.7 * len(dataset))

    train_data = dataset[:ratio]
    test_data = dataset[ratio:]
        ...
        for row in rows:
            ...
            print(
                ...
                classifier.classify(
                    dict(
                        [token, True]
                        for token in remove_noise(word_tokenize(query_text))
                    )
                ),
            )
    elif mode == "titles":
        for row in rows:
            ...
            print(
                ...
                classifier.classify(
                    dict(
                        [token, True]
                        for token in remove_noise(word_tokenize(title_text))
                    )
                ),
            )
